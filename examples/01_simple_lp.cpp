...
int main() {
    ...
    SCIP_CALL( SCIPcreateConsBasicLinear(scip, &cons, "capacity", 
                      ...
                      nullptr,     // variable array (we'll set this)
                      nullptr,     // coefficient array
                      -SCIPinfinity(scip),  // lower bound
                      3.0) );      // upper bound
    
    // Add variables to constraint
    SCIP_CALL( SCIPaddCoefLinear(scip, cons, x, 1.0) );  // +1.0*x
    SCIP_CALL( SCIPaddCoefLinear(scip, cons, y, 1.0) );  // +1.0*y
